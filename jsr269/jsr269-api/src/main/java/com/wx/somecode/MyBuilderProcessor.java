...
public class MyBuilderProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        Set<? extends Element> elementsAnnotatedWith = roundEnv.getElementsAnnotatedWith(MyBuilder.class);
        elementsAnnotatedWith.forEach(element -> {
            ...
            element.getEnclosedElements().forEach(field -> {
                if (field.getKind() == ElementKind.FIELD) {
                    ...
                    MethodSpec methodSpec = MethodSpec.methodBuilder("build" + transformedName)
                            .returns(TypeName.get(element.asType()))
                            .returns(ClassName.get("com.wx.somecode", builderClassName))
                            .addModifiers(Modifier.PUBLIC)
            ...
            try {
                JavaFile.builder("com.wx.somecode", typeSpec).build().writeTo(System.out);
                JavaFile.builder("com.wx.somecode", typeSpec).build().writeTo(filer);
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
